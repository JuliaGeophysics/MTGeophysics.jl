# Example: interactive lat/lon map-view depth-slice viewer for a ModEM model,
# with shapefile overlays and support for ANY projected coordinate system.
# Author: @pankajkmishra
# This is the generalized-CRS variant of plot_model_XY_with_shapefiles.jl: the model
# is georeferenced from a ModEM data file and displayed in any EPSG coordinate
# system, so the same script works for surveys anywhere in the world.
#
...
#       "EPSG:3005"   — NAD83  / BC Albers (British Columbia)
#
...
#   julia --project=. examples/plot_model_LL_with_shapefiles.jl [model_file] [data_file]
model_file = length(ARGS) >= 1 ? ARGS[1] : joinpath(@__DIR__, "geoenergialoikka", "best_model_chain01.rho")
data_file  = length(ARGS) >= 2 ? ARGS[2] : joinpath(@__DIR__, "geoenergialoikka", "data.dat")   # needed for any CRS except "model"

# ---------- Coordinate system ----------
# Choose how the map axes are labelled and how model centres are converted:
#   "EPSG:4326"  — WGS 84 geographic (Longitude / Latitude in degrees)  ← default
#   "EPSG:3067"  — ETRS-TM35FIN (Easting / Northing in metres)
...
shapefiles = [
   # (path = joinpath(@__DIR__, "gis", "2023MeasPts", "2023MeasPts.shp"), enabled = true,  color = :black, alpha = 0.9, point_size = 7,  line_width = 1.5),
    (path = joinpath(@__DIR__, "geoenergialoikka","MTSites", "MTSites.shp"),              enabled = true,  color = :black,   alpha = 0.8, point_size = 5,  line_width = 2.0),
]
...
# ---------- GIS export settings ----------
gis_output_dir  = ""              # "Export GIS" output directory; "" → "<model>-GIS" in the current directory
qml_style_file  = joinpath(dirname(@__DIR__), "src", "MTSlices.qml")    # QGIS style copied with the shapefiles
lyrx_style_file = joinpath(dirname(@__DIR__), "src", "MTSlices.lyrx")   # ArcGIS Pro style copied with the shapefiles
...
# ---------- Plain axis tick labels (no scientific notation) ----------
...
function depth_slice_viewer(
    ...
    function export_figure()
        ...
        filename = "$(model_name)_LL_layer$(layer_idx)_$(view_type)_$(timestamp).png"
