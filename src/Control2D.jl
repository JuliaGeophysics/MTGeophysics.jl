...
# fwd.ctrl, inv.ctrl and the model covariance file used by the 2D forward and inversion workflows
...
_ctrl_algorithm(s) = (a = Symbol(lowercase(strip(s))); a in (:gn, :nlcg, :vfsa) ? a :
                      throw(ArgumentError("algorithm must be GN, NLCG or VFSA")))
...
Inversion control, read from `inv.ctrl`. The lambda, rms and iteration keys follow
ModEM's NLCG control file; the rest are MTGeophysics extras with defaults.
- `algorithm`: `:gn`, `:nlcg` or `:vfsa`
...
- `gn_damping`, `nlcg_restart`, `nlcg_precondition`, `vfsa_*`: per-algorithm settings
"""
Base.@kwdef struct InvCtrl2D
    ...
    vfsa_chains::Int = 2
    vfsa_control_points::Int = 400
    vfsa_trials::Int = 1
    vfsa_step_scale::Float64 = 0.11
    vfsa_seed::Int = 20260308
end
...
const _INV_CTRL_SPEC = (
    ...
    "VFSA chains"                        => (:vfsa_chains, _ctrl_int, false),
    "VFSA control points"                => (:vfsa_control_points, _ctrl_int, false),
    "VFSA trials"                        => (:vfsa_trials, _ctrl_int, false),
    "VFSA step scale"                    => (:vfsa_step_scale, _ctrl_float, false),
    "VFSA seed"                          => (:vfsa_seed, _ctrl_int, false),
)
...
Read a 2D inversion control file.
"""
...
Write a 2D inversion control file with the shared keys and those of its algorithm;
the keys of the other algorithms are left out and read back as defaults.
"""
function WriteInvCtrl2D(path::AbstractString, c::InvCtrl2D)
    ...
    if c.algorithm == :vfsa
        append!(rows, [
            ("VFSA chains", string(c.vfsa_chains)),
            ("VFSA control points", string(c.vfsa_control_points)),
            ("VFSA trials", string(c.vfsa_trials)),
            ("VFSA step scale", g(c.vfsa_step_scale)),
            ("VFSA seed", string(c.vfsa_seed)),
        ])
    else
        append!(rows, [
...
- `mask`: `(nz, ny)` earth cells, top row first; 0 = air or fixed, 1 = free,
  2 and up = exception classes, inverted like 1
"""
