...
# Ensures MakeMesh2D writes consistent inputs (models, cov.ctrl and mask.ctrl agree, air is mask 0, stations sit on
# the ground), Mask2D fixes the requested cells, and the headless window rebuilds on every slider, toggles core and
# full views and saves what batch mode writes
...
@testset "2D mesh tool" begin
    ...
    data = DataFile2D(title = "", periods = 1 ./ f, frequencies = f, site_names = ["TK" * lpad(i, 2, '0') for i in 1:ns], receivers = y,
    ...
    mktempdir() do dir
        ...
        # station columns take the boundary nearest their station, up or down, so none is buried or floating
        mesh, _ = Mesh2DFromInputs(mt, observed, fwd; warn = false)
        @test all(o -> abs(o.offset) <= o.tolerance, mt2d_station_offsets(mesh, observed))
        @test !any(n -> occursin("snap by more than half a cell", n), t.notes)
        @test isfile(joinpath(dir, "topo", "MeshCore.png")) && fwd.strike === nothing
        @test !isfile(joinpath(dir, "topo", "data-r.dat"))                  # off-diagonal data: no rotation
        ...
        ctrls = (inv = joinpath(pkgdir(MTGeophysics), "examples", "ctrl", "2D", "InvCtrl.GN"),
                 vfsa = joinpath(pkgdir(MTGeophysics), "examples", "ctrl", "2D", "InvCtrl.VFSA"), rotated = "")
        ...
        # core/full view toggle, as in MakeMesh3D
        core_x = w.axis.targetlimits[].widths[1]
        w.view_button.clicks[] += 1
        @test w.view_button.label[] == "Show core" && w.axis.targetlimits[].widths[1] > 2core_x
        w.view_button.clicks[] += 1
        @test w.view_button.label[] == "Show full" && w.axis.targetlimits[].widths[1] ≈ core_x
